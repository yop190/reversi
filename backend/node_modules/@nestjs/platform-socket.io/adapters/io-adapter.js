    ...
    bindMessageHandlers(socket, handlers, transform) {
        ...
        handlers.forEach(({ message, callback, isAckHandledManually }) => {
            const source$ = (0, rxjs_1.fromEvent)(socket, message).pipe((0, operators_1.mergeMap)((payload) => {
                ...
                return transform(callback(data, ack)).pipe((0, operators_1.filter)((response) => !(0, shared_utils_1.isNil)(response)), (0, operators_1.map)((response) => [response, ack, isAckHandledManually]));
            ...
            source$.subscribe(([response, ack, isAckHandledManually]) => {
                ...
                if (!isAckHandledManually && (0, shared_utils_1.isFunction)(ack)) {
                    ack(response);
                }
            });
        });
